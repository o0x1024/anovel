...
export const ANALYSIS_SLOT_ANCHORS: Partial<Record<string, IncubatorSlotKey[]>> = {
  variants: [],
  expand: ['core_conflict'],
  world_rules: ['core_conflict', 'hook'],
  role_engine: ['core_conflict', 'hook', 'world_rules'],
  rhythm_curve: ['core_conflict', 'hook', 'role_engine', 'world_rules'],
  ending_structure: ['core_conflict', 'hook', 'rhythm_curve'],
  diagnose: [...INCUBATOR_SLOT_KEYS],
  reverse: [...INCUBATOR_SLOT_KEYS],
  anchors: ['core_conflict', 'hook'],
  benchmark: ['core_conflict', 'hook'],
  tone: ['core_conflict', 'hook', 'rhythm_curve'],
  frontstory: ['core_conflict', 'hook'],
  microinnovation: ['core_conflict', 'hook', 'world_rules']
}
...
> = {
  variants: {
    ...
    sourceStep: 'variants',
    system: [
      '你是顶级的网文故事结构策划师与脑洞拓展大师。基于用户提供的【创作种子】，请彻底打破常规套路，进行极限发散，沿 6 个截然不同且极具颠覆性的创新维度，为故事寻找最硬核的【核心冲突】。',
      '【发散维度建议】',
  ...
  expand: {
    ...
    sourceStep: 'expand',
    system: [
      '你是顶级的网文故事结构策划师与大纲推演专家。基于用户已确定的【核心冲突】（主线槽位），请进行深度推演，生成 3 条截然不同、互不重合的【开局钩子】与故事发展路径。',
  ...
  role_engine: {
    ...
    system: [
      '生成 3 套极其饱满完善的角色驱动方案。须与已给主冲突轴、世界规则轴完美契合，确保角色动机足以支撑整个故事的烈度，且在世界规则的压力系统下做出合理选择。',
  ...
  world_rules: {
    ...
    system: [
      '生成 3 套逻辑严密、设定丰满的世界规则方案。须与已给前台钩子、主冲突轴高度一致。规则必须具有强烈的压迫感，能逼迫主角做生死抉择，并至少一次严重反噬主角。',
  ...
  rhythm_curve: {
    ...
    system: [
      '你是顶级的网文节奏设计大师。基于已确认的主冲突轴、前台钩子、世界规则轴、角色驱动轴，设计 3 套完整的读者情绪节奏曲线与章节节拍密度方案。',
      '【强一致性警告】',
      '节奏设计必须服务主冲突的推进与角色成长弧，而非单独存在。每个转折点须绑定具体的剧情事件与场景细节，绝对禁止空泛形容词堆砌。',
      '【输出要求】',
      '每套方案须包含以下字段：',
  ...
  ending_structure: {
    ...
    system: [
      '你是顶级的网文终局架构师。基于已确认的主冲突轴、前台钩子、节奏曲线轴，设计 3 套饱满且具有强烈感染力的终局结构方案。',
      '【核心理念】',
      '终局不是单一定格画面，而是包含冲突收束、角色弧闭合、世界状态定局、读者离场情绪的完整结构。须让主角付出不可逆代价或做出不可撤回的终极选择。',
      '【输出要求】',
      '每套方案须包含以下字段：',
      '- title：极具感染力的终局方案命名',
      ...
      '  ④ 终局画面：极其详尽的电影级镜头描写（包含光影、色彩、声音、动作等细节，定格最震撼的瞬间）。',
      '  ⑤ 读者离场情绪与主题升华：精准设计读者看完最后一段的情绪反应（头皮发麻/泪流满面/细思极恐/怅然若失/热血沸腾），并通过具体的终局场景细节、台词或意象来实现该效果；同时说明全书核心主旨如何通过终局事件得到最终诠释。',
      INCUBATOR_ANTI_MEDIOCRITY,
      JSON_ONLY,
      '注意：summary 字段必须是一个完整的字符串，不要将其拆分为数组或多个字段。请使用 \\n 来换行。',
...
export const INCUBATOR_DIAGNOSE_APPLY_SYSTEM = [
  ...
  '5) slotKey 只能是：core_conflict、hook、world_rules、role_engine、rhythm_curve、ending_structure。',
  INCUBATOR_ANTI_MEDIOCRITY,
...
export const INCUBATOR_GATE_CHECK_SYSTEM = [
  '你是资深网文结构编辑与极其苛刻的逻辑推敲专家，负责对「主线六槽」进行严格的门禁判定。',
  '【核心判定理念】',
  '你必须从整体出发，把六个槽位视作一个完整的、连续的短故事大纲。故事是精密耦合的齿轮系统，不能孤立、割裂地审核单个槽位。你必须先对全局逻辑和结构进行推演，再将发现的具体问题定位并推送到对应的槽位中。',
  ...
  '4) coherence 仅列跨槽一致性问题，slotKey 只能是 core_conflict/hook/world_rules/role_engine/rhythm_curve/ending_structure，severity 只能是 blocking 或 warning。',
  ...
  '   - **极其重要**：original 必须从上方【主线六槽】原文中**逐字逐标点复制粘贴**，禁止凭记忆改写、禁止省略任何标点符号或空格、禁止调整措辞。哪怕原文有错别字也要原样保留。',
...
export function buildGateCheckUserPrompt(
  ...
  const parts = [
    '【任务】请依据下列六槽内容进行门禁判定，并为每个阻断/警告项提供可直接应用的 replacements（局部替换）。',
    ''
  ]
  if (seed?.trim()) {
    parts.push('【创作种子】', seed.trim(), '')
  }
  parts.push('【主线六槽】', buildSlotAnchorBlock(slots, [...INCUBATOR_SLOT_KEYS]) || '（各槽均为空）')
  if (characters && characters.length > 0) {
...
export const INCUBATOR_GATE_FIX_SYSTEM = [
  ...
  '规则：',
  '1) 全局连动：必须修复所有阻断项。若修复动作改变了某个核心设定，你必须同步检查并修改其他槽位（哪怕它原本是 passed 的），确保整个六槽逻辑依然闭环。',
  ...
  '6) slotKey 只能是：core_conflict、hook、world_rules、role_engine、rhythm_curve、ending_structure。',
  INCUBATOR_ANTI_MEDIOCRITY,
  '只输出一个 JSON 对象，禁止其它文字：',
  '{"logic_rebuild":"（全局逻辑重构推演，200字以内，写明蝴蝶效应和跨槽联动）", "patches":[{"slotKey":"hook","text":"合并后的完整槽位内容..."}]}',
...
/** 按用户微调指令修订六槽（输出 JSON patches） */
export const INCUBATOR_TWEAK_SYSTEM = [
  '你是主线编排编辑。根据用户的「微调指令」，在现有六槽内容上做精准修订。',
  '规则：',
  ...
  '5) slotKey 只能是：core_conflict、hook、world_rules、role_engine、rhythm_curve、ending_structure（禁止使用 main_conflict 等别名）。',
  '6) patches 仅包含实际需修改的槽位；无变更的槽位不要输出。',
  INCUBATOR_ANTI_MEDIOCRITY,
  '只输出一个 JSON 对象，禁止其它文字：',
  '{"patches":[{"slotKey":"hook","text":"修订后的完整槽位内容..."}]}',
