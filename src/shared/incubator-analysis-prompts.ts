 ...
 *
 * 顺序基于分析依赖链：
 *   premise（无依赖）→ core_conflict（依赖 premise）→ opening（依赖 premise + core_conflict）
 *   → world_rules（依赖 premise + core_conflict + opening）
 *   → role_engine（依赖 premise + core_conflict + opening + world_rules）
 *   → ending（依赖全部前序槽位）
 */
