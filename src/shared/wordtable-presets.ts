...
export interface WordTablePreset {
  ...
  name: string
  description: string
  entries: Array<{ type: 'word' | 'regex'; source: string; target: string }>
}
...
const PRESET_GENERAL: WordTablePreset = {
  ...
  entries: [
    ...
    // ── 正则句式 ──
    { type: 'regex', source: '他的目光([\s\S]{1,40}?)落在([\s\S]{1,40}?)上', target: '他瞄了一眼$2|他看向$2|他盯着$2' },
    { type: 'regex', source: '一股([\s\S]{1,40}?)涌上心头', target: '$1冒了上来|心里一阵$1|一阵$1油然而生' },
    { type: 'regex', source: '不是([\s\S]{1,40}?)而是', target: '$2|$1不重要，$2' },
    { type: 'regex', source: '虽然([\s\S]{1,40}?)但是', target: '固然$1，但$1|$1归$1，可$1' },
    { type: 'regex', source: '不仅([\s\S]{1,40}?)而且', target: '不但$1，更$1|$1不说，$1也' },
    { type: 'regex', source: '正是因为([\s\S]{1,40}?)所以', target: '因为$1，这才$1|就是$1，所以$1' },
  ],
}
