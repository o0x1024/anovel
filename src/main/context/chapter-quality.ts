...
'1. 模糊化数值：将【机械精确度量】的数字转化为感性模糊的状态描述（如"震颤两次"改为"眼底微微一颤"，"敲了三下"改为"不轻不重地叩击"）。对与主线无关的数量（人数/步数/时长/金额等），用概数替代：优先选择"几/余/数/许多/不少/好些"等中文自然概数词，具体词频需匹配情节氛围（紧张场景用"转眼间/片刻"而非"等了一会儿"）；精确数字只在作为情节关键信息时保留。',
  ...
  '若正文末尾附带了【章节上下文】，对以下子维度逐项评估。F1-F4 归入 content_logic 评分，F5 归入 setting_consistency 评分；若未提供章节上下文，content_logic 给满分；若未提供核心设定，setting_consistency 给满分。',
  '',
  '**F1. 情节因果**',
  ...
  '- 角色对话风格是否与其身份/性格匹配？',
  '- **角色状态快照比对（极重要）**：若提供了【角色状态快照】，逐条比对正文是否与快照矛盾。重点检查：位置是否一致（上章在A地，本章无故在B地）、心理状态是否连贯、已知信息是否错乱（角色不应知道尚未获取的信息）、数值状态是否矛盾（如快照体力50，正文却表现为体力80；快照信用度87点，正文却写65点）。每发现一处矛盾，须在 top_issues 中标注具体矛盾点及正文位置。',
  '',
  '**F3. 时空逻辑**',
  ...
  '- 是否出现与已有设定矛盾的新设定？',
  '- **金手指/能力限制检查（极重要）**：若提供了【金手指硬性约束】，逐条比对正文是否违反冷却时间、次数上限、消耗代价、失效场景、反噬机制、红线禁用场景、外人可见性、呈现形式、交互方式等限制。典型违规：设定"一天只能用一次"但正文连续使用、设定"消耗大量精力"但正文无代价地反复使用、设定"对XX无效"但正文对XX生效、设定"面板仅主角可见"但正文写了其他角色看到面板或与面板互动、设定"使用时无外在表现"但正文写了光芒/声音被旁人察觉、设定"需意念触发"但正文写了语音指令。每发现一处违规，须在 top_issues 中标注具体违规点及正文位置。',
  '- 是否偏离主线设定中的故事轨迹与关键转折点？',
  '',
  ...
  '',
  '### 总分',
  '- 总分 score_total：11 项单项分的算术平均值（四舍五入取整，0-100）',
  '- 是否硬性失败：true/false',
  '',
  '### 11项评分（每项独立 0-100 分）',
  '1) AI句式占比（0-100）：含E节所列全部AI典型句式（"不是A而是B"/连接词/模板情感句/动作插入对话/单段多句号/过度修辞等），总计<=1处满分',
  '2) 对话密度（0-100）：对话字数占比 25%-40% 满分',
  '3) 句长波动（0-100）：句长有明显起伏，连续同构长句不超过2句',
  '4) 短句占比（0-100）：<=8字短句占比达到阈值',
  '5) 强意象复用（0-100）：同一强意象每章<=2次',
  '6) 场景描写克制（0-100）：单次纯环境描写<=1句；同一自然意象<=1次；纯环境占比<=5%',
  '7) 推进信息密度（0-100）：每300-500字至少1个推进信息',
  '8) 大纲覆盖度（0-100）：必写节点全部落地',
  '9) 内容逻辑（0-100）：F1-F4子维度综合评估，无逻辑矛盾满分；未提供章节上下文时给满分',
  '10) 设定一致性（0-100）：F5维度评估，违反金手指/能力限制或世界观规则则低分；未提供核心设定时给满分',
  '11) 字数达标（0-100）：偏差<=10%满分，10%-25%扣分，>25%低分；未提供目标字数时给满分',
  '',
  ...
  '- 严重违反金手指/能力限制（如设定"一天一次"但连续使用且无任何代价或解释）',
  '',
  '### 评分一致性硬约束',
  '1. 总分必须等于 11 项分数的算术平均值（四舍五入取整）。',
