...
export const QUALITY_AI_SYSTEM_PROMPT = [
  ...
  '',
  '【段落功能与解释闭合】',
  '- 检查是否连续使用“环境→反应→解释→背景→推进”的完整模板，或在每个动作后立即说明原因与意义。故事因果必须成立，但正文表达允许延后解释。',
  '',
...
'1. 模糊化数值：将【机械精确度量】的数字转化为感性模糊的状态描述（如"震颤两次"改为"眼底微微一颤"，"敲了三下"改为"不轻不重地叩击"）。对与主线无关的数量（人数/步数/时长/金额等），用概数替代：优先选择"几/余/数/许多/不少/好些"等中文自然概数词，具体词频需匹配情节氛围（紧张场景用"转眼间/片刻"而非"等了一会儿"）；精确数字只在作为情节关键信息时保留。',
  ...
  '',
  '### 11项评分（每项独立 0-100 分）',
  '1) AI分布特征（0-100）：综合模板句式、段落功能重复、信息密度均匀和即时解释闭合；不得按单一词语或句号数量直接定性',
...
'9. 段落与句式约束：删除或改写模板化“不是A，而是B”句式；只合并确属同一连续动作的机械碎句，不设置句号数量；故事因果成立但表达原因可以延后。',
  '',
  '【例外：章末钩子修复】',
