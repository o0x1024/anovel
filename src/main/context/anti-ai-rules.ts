...
export const SURFACE_ANTI_AI_PRESETS: AntiAiPreset[] = [
  ...
  },
  {
    ...
    demo: {
      before: '老太太端起茶杯，没喝。碎瓷溅上张三的脚踝时，她没躲。',
      after: '老太太端起茶杯吹了吹浮沫，又原样搁回了桌上。碎瓷溅上脚踝，张三连眼皮都没眨一下。'
    }
  },
  {
    label: '禁机械精确度量',
...
export const DEEP_ANTI_AI_PRESETS: AntiAiPreset[] = [
  {
    label: '注意力留白',
    rule: '段落由视角人物当下真正注意到的事物驱动。允许某些段落只处理琐碎动作、事务或无结论的感受，不要求每段都承担解释、推进和情绪升华',
    demo: {
  ...
  {
    label: '控制修辞密度',
    rule: '修辞必须来自角色的经验和当前注意，不能为了文学感平均分配。没有角色化来源的比喻直接不用，局部是否密集由场景决定，不设置机械数量',
    demo: {
  ...
  {
    label: '词汇意外性',
    rule: '词汇服从人物生活经验和眼前物件；只有角色确实会这样称呼时才使用方言、专业词或具体品牌，不为制造低概率而强塞生僻词',
    demo: {
  ...
  {
    label: '段落节奏变化',
    rule: '一个段落只跟随一次局部注意或反应，完成后自然换段；不要让每段固定执行“环境→反应→解释→推进”，也不要为制造变化而机械插入长句、短句或碎片句',
    demo: {
  ...
  {
    label: '叙述温度起伏',
    rule: '情绪强度由人物注意和事件余波自然变化；高潮后可以继续动作、转向事务或停在空白中，不要自动追加解释、总结，也不要机械插入降温段',
    demo: {
  ...
  {
    label: '打破完美感',
    rule: '允许人物没有想明白、记忆含糊、话未说完或注意转移，但只能由情境和人物状态造成；不得随机制造病句、错字、重复或假装混乱',
    demo: {
...
const HUMAN_WRITING_META = [
  '【正文生成分布原则】',
  'A. 先决定视角人物此刻注意什么、误判什么、避开什么，再决定写进段落的内容；不要先完整描述环境。',
  'B. 一个段落只完成一次局部感知、动作、话语或念头，不负责同时交代全部背景、原因和意义。',
  'C. 故事层因果必须成立，但表达层可以只给动作或结果，把原因留到后文；不要每次都立即解释。',
  ...
  'J. 保留范文中真实存在的重复、偏好和信息失衡，不额外制造病句、错字、生僻词或随机口语。',
  '目标：改变内容被选择和组织的过程，而不是在完成的AI文本表面添加“人味”。'
].join('\n')

const BODY_GENERATION_ANTI_AI_LIGHT = [
  '【正文生成分布约束】',
  '- 禁止“不是A，而是B/不是A，是B”、书面连接词堆砌、模板情感总结和逐帧电影镜头链。',
  '- 从人物当下注意、偏见和回避出发选择细节；一个段落只写一次局部反应，不自动补齐环境、背景、原因和意义。',
  '- 故事因果在事实层成立即可，表达层允许先写动作或结果、延后解释；不要持续形成“动作→原因→说明”的闭合链。',
  '- 句长、段落和信息密度由当前注意自然变化，不为制造“人味”故意写病句、碎句、长逗号链、口语或生僻词。',
...
export const STYLE_REWRITE_INSTRUCTION = [
  '你是资深网文编辑。只做局部表达修复，不对全文统一润色。',
  '',
  '核心目标：不改剧情与人设，不重排主事件；未命中具体问题的原句必须原样保留。',
  '',
  ...
  '',
  '其他编辑原则：',
  '1. 不设置破折号、句号、句长、口语词或生僻词的数量指标。',
  '2. 不随机制造冗余、病句、错字、断裂感或质量落差。',
  '3. 优先删除即时解释：故事因果保持成立，但动作后的原因和意义可以留到后文。',
  '4. 若段落同时完成环境、反应、解释、背景和推进，只保留视角人物当下真正注意到的一条局部路径。',
  '5. 角色原有的重复、口头习惯、模糊记忆和注意跳转可以保留，不得凭空添加。',
  '6. 修复完成后逐段与原文核对，未命中问题的文字保持原样，禁止全篇同义改写。',
  '',
  '【章末钩子例外】',
