...
\`\`\`json
{
  "settingPatches": [
    ...
    { "slot": "golden_finger", "label": "金手指系统", "content": "修订后的金手指 Markdown 摘要", "structuredContent": { "nameAndForm": "...", "abilities": [...], ... } }
  ]
}
\`\`\`
...
- label 为该槽位的中文名称
- content 为 Markdown 文本，用于展示
- **金手指 slot（golden_finger）特殊要求**：必须额外提供 structuredContent 字段，值为完整的金手指结构化 JSON，字段如下：
  - nameAndForm (string): 能力名称与形态
  - manifestation (string): 呈现形式（以什么形式呈现给主角，感官通道，触发方式）
  - visibility (string): 外人可见性（金手指本身是否可见、使用时是否有外在表现、世界观检测手段能否发现）
  - interaction (string): 交互方式（主角如何调用、金手指是否有自主意识/人格）
  - abilities (array): [{ name, effect, scope }]
  - acquisition (string): 获取与觉醒条件
  - sourceNature (string): 来源性质（本质来源、世界观中是否有同类、是否可被夺取/封印）
  - limit (object): { cooldown, cost, usageLimit, invalidScenes }
  - backlash (string): 反噬/代价机制
  - upgrades (array): [{ stage, condition, unlocks }]
  - infoAdvantage (string): 信息差优势
  - sideEffects (string): 副作用/负面绑定
  - forbiddenScenes (string): 禁用/红线场景
  - exposureConsequence (string): 暴露后果（被他人发现的后果、是否需要隐藏、是否有势力针对持有者）
  - tagline (string): 番茄一句话卖点
  - firstPayoffScene (string): 前三章首次爽点场景
  - visualMetric (object): { currentLevel, costPerUse, cooldown, usageCap, progressBar, failureScene }
  - 只输出需要修改的字段，未修改的字段可不填（系统会自动合并已有数据）
- **其他 slot（protagonist / pleasure_engine / world_pressure / conflict_engine / supporting_cast）特殊要求**：content 必须是该槽位的**完整修订后内容**，而非仅修改片段。系统会直接用 content 覆盖原设定，如果只输出修改片段会导致原有信息丢失。
