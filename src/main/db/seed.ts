...
6. 重复与韵律可适度使用，增强记忆点`,
  ...
  for (const style of builtins) {
    ...
    const existing = writingStyleDAO.getByName(style.name)
    if (!existing) {
      ...
      inserted++
    } else if (stepRulesJson && !existing.step_rules_json) {
  ...
  if (inserted > 0) {
    console.log(`[DB] Seeded ${inserted} built-in writing styles`)
  }
  if (patched > 0) {
