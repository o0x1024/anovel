...
const aiSystemPrompts: Record<SettingType, string> = {
  ...
  golden_finger: [
    ...
    '- 番茄铁律：限制必须可感知、可演绎；可用数值/进度条，也可用体感、场景边界、能力阶段和失效表现呈现。若用户要求无数值，禁止强行设计百分比/进度条/固定冷却',
    '输出以下 Markdown 章节：',
    '  ## 番茄一句话卖点',
    '  ## 名称与形态',
    '  ## 呈现形式',
    ...
    '- 外人可见性：金手指本身是否被外人可见？使用时是否有外在表现（光芒/声音/气息波动/身体变化）？世界观中的检测/鉴定手段能否发现金手指存在？这是信息差和装逼打脸的基础',
    '- 交互方式：主角如何调用金手指（意念/语音指令/手势/自动触发）？金手指是否有自主意识或人格？会主动干涉主角决策吗？',
  ...
  main_plotline: [
    '你是顶级的故事主线架构师。基于以下全部核心设定，设计故事从开局到终局的发展轨迹。',
    '若上下文含「用户补充要求」，须严格遵守。',
    '核心原则：',
    '- 主线设定是分卷大纲的"骨架约束"——它决定了故事怎么走，分卷大纲决定每卷怎么展开',
    '- 前 6 类设定是静态元素（谁/什么世界/什么冲突），主线设定是动态轨迹（故事怎么走）',
    '- 好的主线让分卷大纲生成有据可依，而非 AI 自由发挥',
    '- 主线不是分卷摘要——主线是更高层的故事骨架，分卷是对骨架的展开',
    '输出要求：',
    '- 用 Markdown 结构化输出：',
    '  ## 故事起点（主角的初始状态与困境）',
    '  ## 核心发展线（3-5 个关键阶段，每阶段标注：触发事件、主角核心选择、阶段结束时状态变化）',
    '  ## 关键转折点（至少 2 次读者预判之外的转向，标注转折的因果逻辑）',
    '  ## 伏笔与回收布局（主要伏笔的埋设阶段与回收阶段）',
    '  ## 高潮设计（终极对决/抉择的核心冲突逻辑，呼应主角致命缺陷与金手指反噬）',
    '  ## 故事终点（主角的最终状态与主题呼应）',
    '  ## 各阶段递进逻辑（为什么前一阶段必然导向后一阶段——因果闭环）',
    '- 总字数 800-1500 字',
    '- 每个阶段必须标注：触发条件、主角的核心选择、该阶段结束时主角的状态变化',
    '- 递进逻辑必须因果闭环：不能是"然后突然..."——每一阶段必须是前一阶段的必然结果',
    '- 禁止写具体章节情节，聚焦于"故事怎么走"的骨架层面',
    '- 必须与前 6 类设定自洽：主角的选择符合其性格驱动力，冲突升级符合冲突引擎设计，爽点节奏符合爽点机制'
  ].join('\n')
}
...
const aiSystemPromptsStory: Record<SettingType, string> = {
  ...
  golden_finger: [
    ...
    '【路径 A：有特殊设定/金手指机制的故事】',
    '若故事存在超自然/特殊信息差设定，输出：',
    '## 番茄一句话卖点 / ## 设定名称与形态 / ## 核心展现机制（如何融入日常冲突） / ## 信息差构建（主角知道什么/别人不知道什么） / ## 限制与紧迫感（避免万能导致失去张力） / ## 可视化限制指标（阶段/体感消耗/使用间隔/失效后果；无数值设定禁止百分比和进度条） / ## 对核心冲突的推动作用 / ## 前三章首次爽点场景',
    '',
    '【路径 B：纯情感/现实向故事（无金手指）】',
...
const GEN_HINTS_META: Record<SettingType, {
  ...
  main_plotline: {
    title: '主线设定',
    desc: '可指定故事走向偏好（如分几个阶段、关键转折方向、结局基调）。留空则依据全部已有设定自动推导。',
    placeholder: '例如：三幕结构；第一卷聚焦觉醒，第二卷复仇，第三卷守护；结局主角隐退…',
    showNamePicker: false
  }
}
  ...
  <div class="w-full min-w-0">
    ...
    <div class="flex items-start justify-between gap-4 mb-6">
      <p class="text-sm text-base-content/50 min-w-0">
        ...
        <template v-else>
          七类核心设定构成网文的故事引擎。推荐按依赖顺序填充：主角 → 金手指 → 世界观压力 → 冲突引擎 → 爽点机制 → 配角功能组 → 主线设定 → 结构化人设卡片 → 爆款书名与导语。主线设定是最后一步，将前 6 类静态设定综合为故事发展轨迹，为分卷大纲提供骨架约束。AI 生成会自动带入已填的依赖设定。
        </template>
      </p>
