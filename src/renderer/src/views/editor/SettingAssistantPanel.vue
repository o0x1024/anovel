...
const MISSING_FIELD_PROMPTS: Record<string, { label: string; prompt: string }> = {
  ...
  '缺少「呈现形式」': { label: '补呈现形式', prompt: '请为当前金手指补全「呈现形式」：金手指以什么形式呈现给主角（系统面板/UI？脑海中的声音？身体纹路？随身空间？特定物品？纯感知？），通过什么感官通道感知，主动调出还是自动弹出。' },
  '缺少「外人可见性」': { label: '补外人可见性', prompt: '请为当前金手指补全「外人可见性」：金手指本身是否被外人可见？使用时是否有外在表现（光芒/声音/气息波动/身体变化）？世界观中的检测手段能否发现金手指存在？' },
  '缺少「交互方式」': { label: '补交互方式', prompt: '请为当前金手指补全「交互方式」：主角如何调用金手指（意念/语音指令/手势/自动触发）？金手指是否有自主意识或人格？会主动干涉主角决策吗？' },
