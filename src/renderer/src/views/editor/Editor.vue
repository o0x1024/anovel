...
const workflowSteps = [
  { key: 'incubator' as const, label: '大岗孵化', icon: 'lightbulb' },
  { key: 'settings' as const, label: '核心设定', icon: 'sliders' },
  { key: 'volumes' as const, label: '分卷大纲', icon: 'book' },
  { key: 'chapters' as const, label: '章节情节', icon: 'list-ol' },
  { key: 'generate' as const, label: '正文生成', icon: 'pen-nib' },
  { key: 'anchors' as const, label: '锚点管理', icon: 'anchor' },
  { key: 'ideas' as const, label: '灵感收集', icon: 'brain' }
]
...
const utilitySteps = [
  ...
  { key: 'materials' as const, label: '素材库', icon: 'box' },
  { key: 'stability' as const, label: '文风稳定性', icon: 'fire' },
  { key: 'taste' as const, label: '品味档案', icon: 'gem' },
  { key: 'images' as const, label: 'AI 配图', icon: 'palette' },
  { key: 'favorites' as const, label: '收藏夹', icon: 'bookmark' }
] as const

const steps = [...workflowSteps, ...utilitySteps]
...
function stepStatusBadge(key: string) {
  if (!workflowSteps.some(s => s.key === key)) return null
...
<template>
  <div class="flex h-full bg-base-100 font-sans animate-fade-in">
    <aside class="w-60 bg-base-200/60 border-r border-base-300 shrink-0 py-5 flex flex-col overflow-hidden select-none">
      ...
      <div class="mt-4 pt-4 border-t border-base-300/70 shrink-0">
        <div class="px-3 mb-3 text-xs font-bold text-base-content/40 uppercase tracking-wider">
          工具
        </div>
