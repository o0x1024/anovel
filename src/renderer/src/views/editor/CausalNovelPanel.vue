...
const snapshot = ref<CausalSnapshot>({ state: null, decisions: [], chapters: [] })
const worldSeed = ref('')
const goal = ref('')
...
const progress = computed(() => {
  ...
  return Math.min(100, Math.round(snapshot.value.chapters.length / target * 100))
})
...
async function load(): Promise<void> {
  const [workInfo, causal, isRunning] = await Promise.all([
...
<template>
  <div class="w-full min-w-0 space-y-5">
    <div class="flex items-start gap-3">
      ...
      <span v-if="state" class="badge badge-sm ml-auto" :class="state.completed ? 'badge-success' : 'badge-primary'">
        状态 r{{ state.revision }}
      </span>
    </div>
    ...
    <template v-else>
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
          ...
          <div class="rounded-lg bg-base-100 border border-base-300 px-3 py-2">
            ...
            <p class="font-semibold mt-1" :class="running ? 'text-primary' : 'text-base-content/70'">
              {{ running ? '模型处理中' : state?.completed ? '已经收束' : '已停止' }}
            </p>
          </div>
        </div>
        <div class="space-y-1">
          <div class="flex justify-between text-xs text-base-content/50">
            <span>已创建 {{ snapshot.chapters.length }} / {{ work?.target_chapters ?? 0 }} 章</span>
            <span>{{ progress }}%</span>
          </div>
      ...
      <section v-if="snapshot.chapters.length" class="card bg-base-200 border border-base-300 p-5 space-y-3">
        <div class="flex items-center justify-between">
          <div>
            <h4 class="font-semibold text-sm">已生成章节</h4>
            <p class="text-xs text-base-content/40 mt-1">正文只能由因果事务链生成和提交，不能绕过状态门禁手动续写。</p>
          </div>
          <span class="badge badge-ghost badge-sm">{{ snapshot.chapters.length }} 章</span>
        </div>
        <div class="divide-y divide-base-300/70 rounded-lg border border-base-300 bg-base-100">
          <div
            v-for="(chapter, index) in snapshot.chapters"
            :key="chapter.id"
